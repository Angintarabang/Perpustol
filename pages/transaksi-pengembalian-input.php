...
function generate_id_pengembalian($db) {
    ...
    $next_id = (int)$data['last_id'] + 1;
    return 'P' . sprintf('%03s', $next_id); // Contoh: P001, P002
}
?>
...
<table id="tabel-input">
    ...
    <tr>
        ...
        <td>
            <select name="idtransaksi" required onchange="this.form.submit()">
                ...
                <?php
                // Hanya tampilkan transaksi yang statusnya 'Dipinjam'
                $q = mysqli_query($db, "SELECT * FROM tbtransaksi WHERE status_pengembalian='Dipinjam' ORDER BY idtransaksi ASC");
                while($t = mysqli_fetch_array($q)){
                    ...
                    echo "<option value='".htmlspecialchars($t['idtransaksi'])."' $selected>
                          ".htmlspecialchars($t['idtransaksi'])." - ".htmlspecialchars($t['idanggota'])." - ".htmlspecialchars($t['idbuku'])." - ".htmlspecialchars($t['tglpinjam'])."
                          </option>";
                }
                ?>
            </select>
    ...
    } // End if(isset($_POST['idtransaksi']))
    ?>

</table>

<?php if(isset($_POST['idtransaksi']) && $trx){ ?>
...
if (isset($_POST['simpan'])) {
    ...
    // 1. Ambil tgl pinjam DAN cek keberadaan data
    $q = mysqli_query($db, "SELECT tglpinjam FROM tbtransaksi WHERE idtransaksi='$idtrx'");
    $trx_data = mysqli_fetch_array($q);
    
    if (!$trx_data) {
        echo "<script>alert('ERROR: Data Transaksi Pinjam tidak ditemukan!'); document.location='index.php?p=transaksi-pengembalian-input';</script>";
        exit;
    }
    ...
    if ($insert_result) {
        // 6. Update Status Transaksi di tbtransaksi
        mysqli_query($db, "UPDATE tbtransaksi SET status_pengembalian='Sudah Kembali' WHERE idtransaksi='$idtrx'");
